...
const MyInput = ({
  ...
  const maxTypeTranslate =
    maxType === 'letter' ? 'karakter' : maxType === 'word' ? 'kata' : maxType;
      ...
      if (maxType === 'word') {
        if (localCount > max) {
          formSetError(
            ...
            {
              type: 'exceedMaxWord',
              message: `Maksimal hanya ${max} kata, sekarang ${localCount}`,
            },
            { shouldFocus: true },
          );
        }
      ...
      {max && showWordCount ? (
        <>
          ...
          <FormHelperText error={errors[name] ? true : false}>
            {errors[name]?.message ?? `${count}/${max} ${maxTypeTranslate}`}
          </FormHelperText>
        </>
      ) : null}
